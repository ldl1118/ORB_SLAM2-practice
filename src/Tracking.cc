    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    int nRGB = fSettings["Camera.RGB"]; // 1:RGB 0:BGR
    mbRGB = nRGB;
    ...
    float fScaleFactor = fSettings["ORBextractor.scaleFactor"]; // 图像金字塔的变化尺度
    int nLevels = fSettings["ORBextractor.nLevels"]; // 尺度金字塔的层数
    int fIniThFAST = fSettings["ORBextractor.iniThFAST"]; // 提取fast特征点的默认阈值
    int fMinThFAST = fSettings["ORBextractor.minThFAST"]; // 如果默认阈值提取不出足够fast特征点，则使用最小阈值 8

    // tracking的过程都会用到mpORBextractorLeft作为特征点提取器  单目只用left
    ...
    // 单目初始化时，会用mpIniORBextractor作为特征点提取器
...
// 输入左目RGB或RGBA图像和深度图
// 1、将图像转为mImGray和imDepth，并初始化mCurrentFrame
// 2、tracking
// 输出世界坐标系到该帧相机坐标系的变换矩阵
...
// Tracking中最主要的函数——Tracking::Track。Track包含两部分：估计运动、跟踪局部地图。
// Tracking部分的主要思路是在当前帧和（局部）地图之间寻找尽可能多的对应关系，来优化当前帧的位姿。
// 这部分主要用了3种模型：匀速运动模型跟踪、跟踪关键帧和重定位。

// [匀速运动模型跟踪]
// 假设物体处于匀速运动，那么可以用上一帧的位姿和速度来估计当前帧的位姿。上一帧的速度可以通过前几帧的位姿计算得到。
// 这个模型适用于运动速度和方向比较一致，没有大转动的情形下，比如匀速运动的汽车、机器人、人等。而对于运动比较随意的目标就会失效。

// [跟踪关键帧]
// 如果匀速模型失效，那么首先可以尝试和最近的关键帧匹配。作者用Bag of Words（BoW）来加速匹配。
// 首先，计算当前帧的BoW，并设定初始位姿为上一帧的位姿；其次，根据位姿和BoW来寻找特征匹配；最后，利用匹配的特征优化位姿。

// [重定位]
// 如果当前帧与最近邻关键帧的匹配也失败了，说明当前帧已经丢了，无法确定其真实位置。此时，只有去和[所有]关键帧匹配，看能否找到合适的位置。
// 首先，用BoW选取若干关键帧作为备选；其次，寻找有足够多的特征点匹配的关键帧；最后，利用特征点匹配迭代求解位姿。如果有关键帧有足够多的内点，那么选取该关键帧优化出的位姿。
// ～～～～～～这里也许有修改的空间，只和最近的几个关键帧匹配，而不是和所有的关键帧匹配。～～～～～～
void Tracking::Track()
{
    // mState为tracking的状态机
    // 状态有：SYSTME_NOT_READY, NO_IMAGE_YET（图像复位或第一次运行）, NOT_INITIALIZED, OK, LOST
    ...
    else// 步骤2：跟踪
    {
        ...
        // 步骤2.1：跟踪 上一帧 || 参考帧 || 重定位
        // Initial camera pose estimation using motion model or relocalization (if tracking is lost)
        // 在viewer中有个开关menuLocalizationMode，它控制是否ActivateLocalizationMode，并最终管控mbOnlyTracking        
        if(!mbOnlyTracking) // mbOnlyTracking等于false表示正常VO模式（有地图更新），等于true表示用户手动选择定位模式
        {
            if(mState==OK) // 初始化成功
            {
                ...
                // 运动模型是空的或刚完成重定位  mnLastRelocFrameId上一次重定位的那一帧
                ...
                bOK = Relocalization(); // 用BoW搜索，用PnP求解位姿
        }
        else // Localization Mode: 至今行跟踪，局部地图不工作
        {
            if(mState==LOST) // 跟丢了
                bOK = Relocalization();
            else
            {
                ...
                // mbVO为true   表明此帧匹配了很少的MapPoints，少于10个，定位即将失败
                if(!mbVO) // 匹配了很多MapPoints，非常好
                {
                    // 如果速度不为0,则使用匀速模型跟踪
                    if(!mVelocity.empty())
                        bOK = TrackWithMotionModel();
                ...
                else // 只匹配了很少的MapPoints，这时既做跟踪，又做定位
                {
        ...
        // 步骤2.2：帧间匹配得到初始姿态后，对local map跟踪，得到更多的匹配，并优化当前位姿
        
        // local map:当前帧、当前帧的MapPoints、当前关键帧与其它关键帧共视关系
        // 在步骤2.1中主要是两两跟踪（用恒速模型跟踪上一帧 || 跟踪参考帧），而2.2中搜集所有的局部MapPoints，
        // 然后将局部MapPoints和当前帧进行投影匹配，得到更多匹配的MapPoints后再次进行Pose优化
        if(!mbOnlyTracking)
        {
            if(bOK)
                bOK = TrackLocalMap();
        ...
        {
           // mbVO为true 表明匹配的MapPoints很少，无法恢复局部地图，因此不执行TrackLocalMap()。

            // 重定位成功
            if(bOK && !mbVO)
                bOK = TrackLocalMap(); // 只有重定位成功，我们才继续用局部地图。
        }
        ...
        if(bOK)
        {
            ...
            // 步骤2.4：清除UpdateLastFrame中，为当前帧临时添加（为了跟踪而增加）的MapPoints。从当前帧中删除这些点
            // 对非单目的情况，需要对上一帧产生一些新的临时MapPoints
            for(int i=0; i<mCurrentFrame.N; i++)
            {
            ...
            // 步骤2.5：清除临时的MapPoints。这里从MapPoints数据库中删除
            ...
            // 删除那些在bundle adjustment中检测为outlier的MapPoints
            for(int i=0; i<mCurrentFrame.N;i++)
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
        }

        // 如果跟踪和relocation都失败了，只能重新Reset
        if(mState==LOST)
        {
    ...
    {
        // 计算相对姿态Tcr: T CurrentFrame ReferenceKeyFrame
...
// 单目的地图初始化
// 并行计算H和F矩阵，选取其中一个模型，恢复出最开始两帧之间的相对姿态以及点云
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        {
            // 步骤1：得到用于初始化的第一帧，初始化共需要两帧。由于不能从单帧得到深度信息，因此需要选两帧以上的图像，估计摄像机姿态并重建出初始的三维点云。
            mInitialFrame = Frame(mCurrentFrame);
            mLastFrame = Frame(mCurrentFrame); // 记录最近的一帧
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());// mvbPrevMatched最大的情况，就是所有特征点都被跟踪上
    ...
    else // 尝试进行初始化
    {
        ...
        // 如果当前帧特征点太少，重新构造初始器。因此，只有连续两帧的特征点个数都大于100，才能继续初始化
        ...
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            // 设置帧的位姿。将初始化的第一帧作为世界坐标系，因此第一帧的变换矩阵为单位矩阵。
            mInitialFrame.SetPose(cv::Mat::eye(4,4,CV_32F));
            // 由Rcw和tcw构造Tcw,并赋值给mTcw
            ...
            // MonocularInitialization()函数得到mvIniP3D。mvIniP3D是cv::Point3f类型的一个容器，是个存放3D点的临时变量，
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // 步骤4：将3D点包装成MapPoints，并和Key Frames联系起来
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        // 步骤4.2：该KeyFrame的哪个特点可以观测到哪个3D点
        ...
        // b.从众多观测到该MapPoint的特征点中选择区分度最高的描述子
        pMP->ComputeDistinctiveDescriptors();
        
        // c.更新该MapPoint的平均观测方向及观测距离的范围
    ...
    // 步骤6：将MapPoints的中值深度归一化，并归一化两帧之间变换
    float medianDepth = pKFini->ComputeSceneMedianDepth(2); // 评估关键帧的场景深度，q=2表示中值
...
// Local Mapping线程可能会替换关键帧中的某些MapPoints，由于tracking中需要用到mLastFrame，这里检查并更新上一帧中被替换的MapPoints
...
{
    // 步骤1：将当前帧的描述子转化为BoW向量   BoW: Bag of Words 词袋
    // 将当前帧的特征点分到特定层的nodes上
    mCurrentFrame.ComputeBoW();

    // 首先将当前帧与参考关键帧作ORB的匹配，如果匹配点足够多，则安装PnP solver
    ORBmatcher matcher(0.7,true);
    vector<MapPoint*> vpMapPointMatches;

    // 步骤2：通过特征点的BoW加快当前帧与参考帧的特征点匹配
    ...
    mCurrentFrame.SetPose(mLastFrame.mTcw); // 用上一次的Tcw设置初值，使PoseOptimization可以更快收敛

    // 步骤4:通过优化3D-2D的重投影误差来获得位姿
    Optimizer::PoseOptimization(&mCurrentFrame);

    // 步骤5：剔除优化后的outlier匹配点（MapPoints），这些是误匹配的点
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
    ...
    return nmatchesMap>=10; // 若>=10，返回1；若<=10，返回0
}
...
void Tracking::UpdateLastFrame()
{
    ...
    cv::Mat Tlr = mlRelativeFramePoses.back(); // l:last  r:reference
    mLastFrame.SetPose(Tlr*pRef->GetPose()); // Tlr*Trw = Tlw w:world
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 步骤1：根据参考关键帧更新上一帧的位姿
    // 对于双目或RGBD，根据深度值为上一关键帧生成新的MapPoints
    // （跟踪过程中需要将当前帧与上一帧进行特征点匹配，将上一帧的MapPoints投影到当前帧可以缩小匹配范围）
    // 在跟踪过程中，去除outlier的MapPoint，如果不及时增加MapPoint会逐渐减少
    // 该函数的功能：增加RGBD/双目相机 上一帧的MapPoints数量
    UpdateLastFrame();

    // 根据匀速模型估计当前帧的位姿。匀速模型认为，这两帧间的相对运动 和 之前两帧的相对运动 是相同的
    ...
    // 步骤4：优化位姿后，剔除outlier的mvpMapPoints
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 如果闭环检测使用了局部地图，则不插入关键帧
    if(mpLocalMapper->isStopped() || mpLocalMapper->stopRequested())
        return false;

    const int nKFs = mpMap->KeyFramesInMap();

    // 步骤2：如果距离上一次插入关键帧的时间太短，则不插入关键帧。
    ...
    // Condition 1c: tracking is weak  跟踪快不行了
    const bool c1c =  mSensor!=System::MONOCULAR && (mnMatchesInliers<nRefMatches*0.25 || ratioMap<0.3f) ;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches. 与最近一个关键帧的重复度不是太高
    const bool c2 = ((mnMatchesInliers<nRefMatches*thRefRatio || ratioMap<thMapRatio) && mnMatchesInliers>15);

    if((c1a||c1b||c1c)&&c2)
    {
        // 如果mapping线程接收了keyframe，则插入keyframe，否则发送信号中断BA
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                // 但是仅仅为了让地图稠密直接改这些不太好，因为这些MapPoints会参与之后整个slam过程
                if(vDepthIdx[j].first>mThDepth && nPoints>100)
                    break;
            }
        }
...
{
    // Reset if cannot Relocalization
    ++mCntRelocs;
    if (mCntRelocs > mMaxRelocs)
    {
        mCntRelocs = 0;
        Reset();
    }
