...
// Initializer文件中的主程序
// 并行计算H矩阵和F矩阵，并选取其中一个模型，恢复出最开始两帧之间的相对姿态以及点云
                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // vAllIndices用于生成0到N-1的数作为特征点的索引
    vector<size_t> vAllIndices;
    vAllIndices.reserve(N); // 用reserve预留N个大小的空间，避免内存重复分配和大量的数据搬移，提高效率。
    vector<size_t> vAvailableIndices;

    for(int i=0; i<N; i++)
        vAllIndices.push_back(i);

    // 步骤2：在所有匹配特征点对中，随机选择8对匹配特征点为一组，共选择mMaxIterations组，用于FindHomography和FindFUndamental函数求解
    // mMaxIterations: 200
...
// 计算单应矩阵H
// 假设场景为平面情况，通过前两帧求取Homography矩阵(current frame 2 到 reference frame 1),并得到该模型的评分
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    // 假定存在的(putative)match数量。
    const int N = mvMatches12.size();
...
// 计算基础矩阵F
// 假设场景为立体，通过前两帧求取Fundamental矩阵(current frame 2 到 reference frame 1),并得到该模型的评分
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    // 假定存在的match的数量
    const int N = vbMatchesInliers.size();

    // 将mvKeys1和mvKeys2归一化到均值为0,一阶绝对矩为1,归一化矩阵分别为T1和T2
    vector<cv::Point2f> vPn1, vPn2;
    cv::Mat T1, T2;
    Normalize(mvKeys1,vPn1, T1);
    Normalize(mvKeys2,vPn2, T2);
